...
(function () {
	 ...
	 */
	function zvgAcfInitChooser(form) {
		...
		/**
		 * Whether the question on screen has an answer.
		 *
		 * @param {number} index Question index.
		 * @return {boolean} True when one of its answers is checked.
		 */
	 ...
	 */
	function zvgAcfInitChoosers() {
		var forms = document.querySelectorAll('[data-chooser]');
		var index;

		for (index = 0; index < forms.length; index++) {
			zvgAcfInitChooser(forms[index]);
		}
	}
	...
	if (document.readyState !== 'loading') {
		...
		document.addEventListener('DOMContentLoaded', zvgAcfInitChoosers);
	}
})();
