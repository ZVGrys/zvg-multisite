...
PHP is checked twice. **PHPCS** against the WordPress Coding Standards — one ruleset per theme,
each pinned to its own text domain, so a domain leaked from another build is an error rather than
a nuance. **PHPStan** at level 5, with stubs for WordPress core, ACF Pro and Elementor.

```bash
cd wp-content/themes
composer install            # first time only
composer check              # both tools, all three themes
composer lint               # PHPCS only
composer analyse            # PHPStan only
```

Both run on every pull request that touches PHP
([`.github/workflows/php.yml`](.github/workflows/php.yml)). All three themes pass clean, and
there is **no baseline file** — nothing is being deferred. Where a rule is deliberately not
